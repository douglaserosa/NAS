 ...
 * @param n [description]
 */
struct threadStruct * epCilk (struct threadStruct * params) {
    double   random_seed;
    unsigned long   j;
    Pair            pair;
    double          t;
...
ep(void)
{
    ...
    for (i = 0; i < NUM_THREADS; i++) {
        ...
        pthread_create(&threads[i], NULL, epThread, (void *) &threadParams[i]);
            ...
            cilk_spawn(epCilk(&threadParams[i]));
        }
    }
...
main(int argc, char * argv[])
{
    ...
    switch (class) {
        case 'W':
            M = 25;
            n = pow(2,M);
            printf("EP-PThreads-Cilk: Class W\n\n");
            break;
        case 'A':
            M = 28;
            n = pow(2,M);
            printf("EP-PThreads-Cilk: Class A\n\n");
            break;
        case 'B':
            M = 30;
            n = pow(2,M);
            printf("EP-PThreads-Cilk: Class B\n\n");
            break;
        case 'C':
            M = 32;
            n = pow(2,M);
            printf("EP-PThreads-Cilk: Class C\n\n");
            break;
        case 'S':
        default:
            M = 24;
            n = pow(2,M);
            printf("EP-PThreads-Cilk: Class S\n\n");
            break;
    }
