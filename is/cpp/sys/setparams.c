...
void check_info(int type, char classe);
void read_info(int type, char *classp);
void write_info(int type, char classe);
void write_sp_info(FILE *fp, char classe);
void write_bt_info(FILE *fp, char classe);
void write_lu_info(FILE *fp, char classe);
void write_mg_info(FILE *fp, char classe);
void write_cg_info(FILE *fp, char classe);
void write_ft_info(FILE *fp, char classe);
void write_ep_info(FILE *fp, char classe);
void write_dc_info(FILE *fp, char classe);
void write_is_info(FILE *fp, char classe);
void write_ua_info(FILE *fp, char classe);
...
int main(int argc, char *argv[])
{
  ...
  get_info(argv, &type, &classe);
  if (classe != 'U') {
#ifdef VERBOSE
    printf("setparams: For benchmark %s: class = %c\n", 
	   argv[1], classe); 
#endif
    check_info(type, classe);
  }
...
void check_info(int type, char classe) 
{
  ...
  if (classe == 'E' && (type == IS || type == UA || type == DC)) {
    printf("setparams: Benchmark class %c not defined for IS, UA, or DC\n", classe);
    exit(1);
  }
  if ((classe == 'C' || classe == 'D') && type == DC) {
...
c  \n");
          break;
      case IS:
          fprintf(fp, DEF_CLASS_LINE, classe);
          fprintf(fp, "\
/*\n\
...
/*\n\
  ...
  switch(type) {
  case SP:
    write_sp_info(fp, classe);
    break;	      
  case BT:	      
    write_bt_info(fp, classe);
    break;	      
  case DC:	      
    write_dc_info(fp, classe);
    break;	      
  case LU:	      
    write_lu_info(fp, classe);
    break;	      
  case MG:	      
    write_mg_info(fp, classe);
    break;	      
  case IS:	      
    write_is_info(fp, classe);  
    break;	      
  case FT:	      
    write_ft_info(fp, classe);
    break;	      
  case EP:	      
    write_ep_info(fp, classe);
    break;	      
  case CG:	      
    write_cg_info(fp, classe);
    break;
  case UA:	      
    write_ua_info(fp, classe);
    break;
  default:
    printf("setparams: (Internal error): Unknown benchmark type %d\n", type);
    exit(1);
...
void write_sp_info(FILE *fp, char classe) 
{
  ...
  char *dt;
  if      (classe == 'S') { problem_size = 12;  dt = "0.015d0";   niter = 100; }
  else if (classe == 'W') { problem_size = 36;  dt = "0.0015d0";  niter = 400; }
  else if (classe == 'A') { problem_size = 64;  dt = "0.0015d0";  niter = 400; }
  else if (classe == 'B') { problem_size = 102; dt = "0.001d0";   niter = 400; }
  else if (classe == 'C') { problem_size = 162; dt = "0.00067d0"; niter = 400; }
  else if (classe == 'D') { problem_size = 408; dt = "0.00030d0"; niter = 500; }
  else if (classe == 'E') { problem_size = 1020; dt = "0.0001d0"; niter = 500; }
  else {
...
void write_bt_info(FILE *fp, char classe) 
{
  int problem_size, niter;
  char *dt;
  if      (classe == 'S') { problem_size = 12;  dt = "0.010d0";   niter = 60; }
  else if (classe == 'W') { problem_size = 24;  dt = "0.0008d0";  niter = 200; }
  else if (classe == 'A') { problem_size = 64;  dt = "0.0008d0";  niter = 200; }
  else if (classe == 'B') { problem_size = 102; dt = "0.0003d0";  niter = 200; }
  else if (classe == 'C') { problem_size = 162; dt = "0.0001d0";  niter = 200; }
  else if (classe == 'D') { problem_size = 408; dt = "0.00002d0";  niter = 250; }
  else if (classe == 'E') { problem_size = 1020; dt = "0.4d-5";    niter = 250; }
  else {
    printf("setparams: Internal error: invalid class %c\n", classe);
    exit(1);
  }
...
void write_dc_info(FILE *fp, char classe) 
{
  ...
  if      (classe == 'S') { input_tuples = 1000;     attrnum = 5; }
  else if (classe == 'W') { input_tuples = 100000;   attrnum = 10; }
  else if (classe == 'A') { input_tuples = 1000000;  attrnum = 15; }
  else if (classe == 'B') { input_tuples = 10000000; attrnum = 20; }
  else {
    printf("setparams: Internal error: invalid class %c\n", classe);
    exit(1);
  }
...
void write_lu_info(FILE *fp, char classe) 
{
  ...
  if      (classe == 'S') { problem_size = 12;  dt_default = "0.5d0"; itmax = 50; }
  else if (classe == 'W') { problem_size = 33;  dt_default = "1.5d-3"; itmax = 300; }
  else if (classe == 'A') { problem_size = 64;  dt_default = "2.0d0"; itmax = 250; }
  else if (classe == 'B') { problem_size = 102; dt_default = "2.0d0"; itmax = 250; }
  else if (classe == 'C') { problem_size = 162; dt_default = "2.0d0"; itmax = 250; }
  else if (classe == 'D') { problem_size = 408; dt_default = "1.0d0"; itmax = 300; }
  else if (classe == 'E') { problem_size = 1020; dt_default = "0.5d0"; itmax = 300; }
  else {
    printf("setparams: Internal error: invalid class %c\n", classe);
    exit(1);
  }
...
void write_mg_info(FILE *fp, char classe) 
{
  ...
  if      (classe == 'S') { problem_size = 32; nit = 4; }
/*  else if (classe == 'W') { problem_size = 64; nit = 40; }*/
  else if (classe == 'W') { problem_size = 128; nit = 4; }
  else if (classe == 'A') { problem_size = 256; nit = 4; }
  else if (classe == 'B') { problem_size = 256; nit = 20; }
  else if (classe == 'C') { problem_size = 512; nit = 20; }
  else if (classe == 'D') { problem_size = 1024; nit = 50; }
  else if (classe == 'E') { problem_size = 2048; nit = 50; }
  else {
...
void write_ft_info(FILE *fp, char classe) 
{
  ...
  if      (classe == 'S') { nx = 64; ny = 64; nz = 64; niter = 6;}
  else if (classe == 'W') { nx = 128; ny = 128; nz = 32; niter = 6;}
  else if (classe == 'A') { nx = 256; ny = 256; nz = 128; niter = 6;}
  else if (classe == 'B') { nx = 512; ny = 256; nz = 256; niter =20;}
  else if (classe == 'C') { nx = 512; ny = 512; nz = 512; niter =20;}
  else if (classe == 'D') { nx = 2048; ny = 1024; nz = 1024; niter =25;}
  else if (classe == 'E') { nx = 4096; ny = 2048; nz = 2048; niter =25;}
  else {
    printf("setparams: Internal error: invalid class type %c\n", classe);
    exit(1);
  }
...
void write_ep_info(FILE *fp, char classe)
{
  ...
  int m;
  if      (classe == 'S') { m = 24; }
  else if (classe == 'W') { m = 25; }
  else if (classe == 'A') { m = 28; }
  else if (classe == 'B') { m = 30; }
  else if (classe == 'C') { m = 32; }
  else if (classe == 'D') { m = 36; }
  else if (classe == 'E') { m = 40; }
  else {
    printf("setparams: Internal error: invalid class type %c\n", classe);
    exit(1);
  }
