...
// #define  TOTAL_KEYS_LOG_2    4
// #define  MAX_KEY_LOG_2       5
...
void	create_seq( int myId )
{
  ...
  chunk = (NUM_KEYS + (NUM_HOSTS * NUM_THREADS) - 1) / (NUM_HOSTS * NUM_THREADS);
  ini = chunk * myId;
  fim = ini + chunk;
  if ( fim > NUM_KEYS ) {
    fim = NUM_KEYS;
  ...
  mySeed = find_my_seed( myId, NUM_HOSTS * NUM_THREADS, (long)4*NUM_KEYS, seed, a );
...
int main( int argc, char **argv )
{
    ...
    INT_TYPE chunk;
    int ini, fim;
    int             i, j, iteration, timer_on, threadid;
    double          timecounter;
    FILE            *fp;
    int myrank;
    MPI_Status      st;
...
/*  Generate random number sequence and subsequent keys on all procs */
    #pragma omp parallel private(threadid)
    {
        ...
        // printf("host = %d, ", myrank);  
        // printf("myId = %d\n", myrank * NUM_THREADS + threadid);
        create_seq(myrank * NUM_THREADS + threadid);
    }

    if (myrank == 0) {
      // sincronizar resultados
      for (i = 1; i < NUM_HOSTS; i++) {
        chunk = (NUM_KEYS + NUM_HOSTS - 1) / NUM_HOSTS;
        ini = chunk * i;
        fim = ini + chunk;
        if ( fim > NUM_KEYS ) {
          fim = NUM_KEYS;
